...
type Cache struct {
	...
	//快照是当前正在写入tsm文件的缓存对象，它们在刷新时保留在内存中，因此可以与缓存一起查询它们。
	snapshot     *Cache
	snapshotting bool

	// This number is the number of pending or failed WriteSnaphot attempts since the last successful one.
	//此数字是自上次成功尝试写入以来或失败的WriteSnaphot尝试次数。
	...
	//用于使用存储初始化缓存的一次性同步。 由于store可以跨分片分配大量内存，我们懒得创建它
	initialize       atomic.Value
	initializedCount uint32
}

// NewCache returns an instance of a cache which will use a maximum of maxSize bytes of memory.
//NewCache返回一个缓存实例，该实例将使用最大maxSize字节的内存。
// Only used for engine caches, never for snapshots. 仅用于引擎缓存，从不用于快照。
